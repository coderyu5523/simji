...
| 9 | 배치·가격 | 중고등 방(`middle`) 노출 + 기관 링크 직접 진입 / **무료**(검사권·결제 불필요) |
| 9b | 로그인 | 개인 직접 응시는 **로그인 필수**(2026-07-02 정책 유지). 비로그인은 **기관 링크 경로만** |
...
### 4.2 경로 1 — 개인 직접 (**로그인 필수**, 무료)

기존 정책 유지: `routes/web.php:32` "검사 진행은 무료·유료 구분 없이 로그인 필수(비회원 응시 불가)" — 2026-07-02 결정. `assessment/*` 라우트의 `auth` 미들웨어를 그대로 둔다. **무료**라는 것은 검사권·결제가 필요 없다는 뜻이지 비로그인을 뜻하지 않는다.

비로그인 응시는 **기관 링크(`/t/{token}`) 경로에서만** 허용된다(§4.3, 현행 `LinkController` 그대로).
...
| 경로 | 재열람 |
|---|---|
| 개인 직접 (항상 로그인) | 내 검사함에 보관, 상시 |
| 기관 링크 (비로그인) | 담당자는 명부에서 상시. 청소년 본인은 세션 유지 동안 — 결과 화면에 "이 링크를 저장해 둬" 안내 |
