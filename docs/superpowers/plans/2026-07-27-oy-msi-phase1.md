...
> 006 도 표지가 `부모·보호자용 및 교사·상담자용 결과보고서`(`006-guardian-report.txt:5`)라
> "부모·보호자가 할 일" 목록 안에 담당자용 줄이 일부 섞여 있다.
>
...
> `FAM.RED.meaning` — 제거하면 키가 비어 rule 상 보류, `LIF.RED.actions` 의 "사례관리",
> `DEP.YELLOW.actions` 의 "권고합니다", 솔루션 제목 `SOL_FAM_PROTECT` = "보호자 안전성 평가·중재").
