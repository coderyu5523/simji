...
> **`SOL_FAM_PROTECT` 카드는 보호자 화면에서 통째로 제외했다(2026-07-28 결정).** 제목
> ("보호자 안전성 평가·중재")은 전문가용, 실천항목("상담자나 믿을 수 있는 어른에게 가족상황 말하기")은
> 청소년용이라 둘 다 보호자용이 아니다. 제목만 가리면 실천항목이 "아이가 외부에 가족 상황을 알리도록
> 안내받고 있다"로 읽혀 문제가 남는다. → `ReportComposer::SOLUTIONS_EXCLUDED_BY_AUDIENCE` 에
> 코드와 이유를 적어 **조립 계층에서만** 제외한다(엔진·`scoring_rules` 무수정, 청소년 화면은 그대로).
> 제외 사실은 섹션의 `excluded_for_audience` 에 남아 조회 실패와 구분된다.
> **저자 확인 대상 추가:** `SOL_FAM_PROTECT` 의 **보호자용 제목·실천항목**(현재 존재하지 않음).
