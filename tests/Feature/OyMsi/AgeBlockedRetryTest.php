...
test('대상 연령이 아니어서 막히면 연령 입력으로 되돌아갈 링크가 있다', function () {
    $this->actingAs($this->user)
        ->post(route('oymsi.age.submit', 'OY_MSI'), ['birthdate' => birthdateAged(19)])
        ->assertOk()
...
test('만 13세 개인 경로 차단에도 연령 입력으로 되돌아갈 링크가 있다', function () {
    $this->actingAs($this->user)
        ->post(route('oymsi.age.submit', 'OY_MSI'), ['birthdate' => birthdateAged(13)])
        ->assertOk()
        ->assertSee(route('oymsi.age.form', 'OY_MSI'), escape: false);
});
...
test('링크 경로 차단은 개인 경로가 아니라 그 링크의 연령 입력으로 돌아간다', function () {
    ...
    $res = $this->post(route('link.age.submit', 'tok-retry'), ['birthdate' => birthdateAged(13)]);
    $res->assertOk();
