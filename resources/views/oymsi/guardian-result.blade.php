{{--
  OY_MSI 보호자용 결과 화면 — 청소년이 만든 공유 링크(/r/{token})로 로그인 없이 열린다.

  $sections 는 ReportComposer 가 audience='GUARDIAN' 으로 조립한 것이고, 순서는
  005 부록1 §2(안전 → 종합 → 영역별 → 상위3 → 강점 → 실천 → 재검 → 고지문)다.
  여기서 순서를 바꾸지 않는다.

  청소년용 화면(oymsi/result.blade.php)과 다른 점은 셋뿐이다.
   1. 어투가 존댓말이고, 보호자 관점의 제목을 쓴다.
   2. GUARDIAN 문안에만 있는 "피해야 할 반응"(avoid)이 상위 3영역마다 붙는다.
   3. 공유 버튼이 없다 — 보호자가 이 결과를 다시 퍼뜨리는 통로를 만들지 않는다.

  ★ SAF(자해·자살 안전) 요인의 원점수·요인명은 여기에도 실리지 않는다. FACTORS 에
    애초에 담기지 않으므로(ReportComposer / included_in_overall=false) 찍힐 곳이 없다.
  ★ 실천 솔루션은 내부 코드(SOL_*)가 아니라 사람이 읽는 제목만 찍는다.
--}}
@php
...
<x-layouts.app title="자녀의 마음상태 결과">
<div class="bg-cream min-h-screen">
  <div class="max-w-2xl mx-auto px-4 py-10 space-y-6">

    <div>
      <p class="text-sm text-navy/50">{{ $test->title_easy }}</p>
      <h1 class="mt-1 text-2xl font-extrabold text-deepgreen">자녀의 마음상태</h1>
      <p class="mt-1 text-sm text-navy/50">
        {{ $attempt->nickname }} · {{ optional($attempt->submitted_at)->format('Y년 n월 j일') }} 검사
      </p>
      <p class="mt-3 rounded-2xl bg-white px-4 py-3 text-xs text-navy/60 ring-1 ring-black/5">
        본인이 직접 공유한 결과입니다. 문항별 응답 내용은 포함되어 있지 않습니다.
        @if($expiresAt)
          이 링크는 {{ $expiresAt->format('Y년 n월 j일') }}까지 열립니다.
        @endif
      </p>
    </div>
    ...
    @foreach($sections as $s)
        ...
        <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-black/5">
          ...
          @if($s['has_safety_alert'])
            {{-- 007 §68 "안전경보가 최종판정보다 우선한다" / §246 "전체 지수는 특정 요인의
                 고위험을 상쇄할 수 있다". 종합 점수는 SAF 를 뺀 9요인 합이라, 안전등급이
                 높아도 종합은 초록으로 나올 수 있다. 보호자가 종합 신호만 보고 위의 안전
                 안내를 상쇄해 읽지 않도록 같은 보정을 보호자 어투로 붙인다. --}}
            <p class="mt-3 rounded-2xl bg-signal-red/10 px-4 py-3 text-sm font-semibold text-signal-red">
              이 종합 신호에는 안전에 관한 문항이 포함되어 있지 않습니다. 위의 안전 안내를 먼저 읽어 주십시오.
            </p>
          @endif
          @if($s['score_status'] !== 'COMPLETE')
            <p class="mt-2 text-xs text-signal-yellow">응답하지 않은 문항이 있어 일부 영역은 참고용으로만 보아야 합니다.</p>
          @endif
      ...
      @elseif($s['type'] === 'FACTORS')
        <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-black/5">
          ...
          <p class="mt-4 text-xs text-navy/40">막대는 0~18점을 100으로 환산한 값입니다.</p>
        </section>
      ...
      @elseif($s['type'] === 'STRENGTH')
        <section class="rounded-3xl bg-mint/30 p-6">
          <h2 class="font-bold text-deepgreen">자녀에게 남아 있는 강점</h2>
      ...
      @elseif($s['type'] === 'SOLUTIONS')
        <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-black/5">
          <h2 class="font-bold text-deepgreen">함께 해볼 수 있는 실천</h2>
      ...
      @elseif($s['type'] === 'RECHECK')
        <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-black/5">
          ...
          <p class="mt-1 text-sm text-navy/70">{{ $s['days'] }}일 뒤에 다시 검사하면 변화를 확인할 수 있습니다.</p>
        </section>
    ...
    <section class="rounded-3xl bg-deepgreen/5 ring-1 ring-deepgreen/10 p-6">
      ...
      <ul class="mt-4 space-y-1 text-sm text-navy/70">
        ...
        <li><b class="text-navy">청소년상담 1388</b> · 24시간 365일 · 전화·문자·온라인. 가출, 학교중단, 가족갈등도 상담할 수 있습니다</li>
        <li><b class="text-navy">경찰 112 · 구급 119</b> · 지금 누군가 다칠 것 같을 때 바로</li>
        <li>
          <b class="text-navy">학교 밖 청소년 지원센터 꿈드림</b> · 상담, 검정고시, 학습·진로·취업 지원,
          건강검진, 자립지원을 받을 수 있습니다. 1388에 문의하면 가까운 곳을 연결해 줍니다
        </li>
      </ul>
    </section>

    {{-- 재공유 버튼은 두지 않는다. 공유 여부·범위는 본인이 정한다. --}}
    <div class="print:hidden">
      <button type="button" onclick="window.print()"
              class="w-full rounded-xl bg-navy/10 py-3 font-semibold">인쇄하기</button>
    </div>
