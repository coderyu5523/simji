  ...
  <div class="bg-cream min-h-[70vh]">
    <div class="max-w-2xl mx-auto px-4 py-12">
      @if($test->consent_required)
        <p class="text-sm text-teal font-semibold">1 동의 · <span class="text-navy/30">2 기본정보 · 3 검사</span></p>
      @else
        <p class="text-sm text-teal font-semibold">1 동의 · <span class="text-navy/30">2 검사</span></p>
      @endif
      <h1 class="text-2xl font-extrabold text-deepgreen mt-2">검사 전 동의</h1>
      <p class="text-sm text-navy/50 mt-1">{{ $test->title_easy }} · 약 {{ $test->duration_min }}분 · {{ $test->item_count }}문항</p>
      ...
      {{-- 안내(intro) 단계를 없애면서 그 화면의 응답 요령을 여기로 옮겼다. --}}
      <div class="mt-4 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-black/5 text-left space-y-3 text-sm text-navy/70">
      ...
      <form method="POST" action="{{ route('assessment.agree', $test->code) }}" class="mt-6">
        ...
        @endif
        <button class="mt-6 w-full rounded-xl bg-deepgreen text-cream py-3.5 font-bold shadow-lg hover:brightness-110 transition">{{ $test->consent_required ? '동의하고 계속' : '동의하고 검사 시작' }}</button>
      </form>
    </div>
  </div>
</x-layouts.app>
